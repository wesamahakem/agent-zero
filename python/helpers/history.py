...
def calculate_output_length(messages: list[OutputMessage], ai_label="ai", human_label="human", strip_images=False) -> int:
    ...
    for o in messages:
        if count > 0:
            total_len += 1 # for "\n"
        total_len += _calculate_stringify_output_length(o, ai_label, human_label, strip_images)
        count += 1
    return total_len


def _calculate_stringify_output_length(output: OutputMessage, ai_label="ai", human_label="human", strip_images=False) -> int:
    label = ai_label if output["ai"] else human_label
    # f'{label}: {content}'
    return len(label) + 2 + _calculate_content_length(output["content"], strip_images=strip_images)
...
def _calculate_content_length(content: MessageContent, strip_images: bool = False) -> int:
    ...
    if isinstance(content, list):
        ...
        # else fall through to json dumps
